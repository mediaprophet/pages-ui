...
const Component = ({ location, inboxHeader, setInboxHeader }) => {
  ...
  let shouldViewHeader = (location.pathname !== "/extra/404" &&
    ...
    !location.pathname.includes("/extra/lock_screen") ? true : false);

  const sideBarStatus = useSelector(state => state.QuickView.openMobileSideBar)

  return (
    <div
      className={`header header-corporate ${sideBarStatus && (location.pathname !== '/') ? 'headerAlign' : ''}`}
      ...
      {location.pathname.includes("/boxed_layout") ?
        <div className="container">
          <div className="header-inner">
            <HeaderContent location={location} inboxHeader={inboxHeader} setInboxHeader={setInboxHeader}/>
          </div>
        </div>
        :
        <HeaderContent location={location} inboxHeader={inboxHeader}  setInboxHeader={setInboxHeader}/>
