...
const EmailList = ({ group, groupIndex, onReadEmail, onSlide, emailListPosition }) => {
    ...
    useEffect(() => {
        ...
        const elements = document.getElementsByClassName("item padding-10 p-l-15 mail-list-item-style active");
        ...
        if (selectEmailStyling !== null && selectEmailStyling === document.getElementById(selectEmailStyling).id) {
            document.getElementById(selectEmailStyling).classList.add('active');
            document.getElementById(selectEmailStyling).style.backgroundColor = '#fffde1';
        }
    }, [selectEmailStyling]);

    const emailItem = group.list.map((listValue, listIndex) =>
        <li key={`${groupIndex}-${listIndex}`}
            className={`item padding-10 p-l-15 mail-list-item-style`}
            id={listValue.id}
            data-email-id={`${groupIndex}-${listIndex}`}
            onClick={() => {
                onReadEmail({ 'id': listValue.id, 'groupIndex': groupIndex, 'listIndex': listIndex })
            ...
            <Row>
                <Col xs={2} md={2}>
                    <div className="thumbnail-wrapper d32 circular flex-shrink-0">
                        <img width="40" height="40" alt="" data-src-retina={listValue.dpRetina} data-src={listValue.dp} src={'../' + listValue.dpRetina} />
                    </div>
                </Col>
                <Col xs={5} md={10}>
                    <Row>
                        <Col xs={7} md={7} className="no-padding">
                            <p className="recipients no-margin">
                                {listValue.to.toString()}
                            </p>
                        </Col>
                        <Col xs={5} md={5}>
                            <div className="datetime email-list-date-class">{listValue.time}</div>
                        </Col>
                    </Row>
                    <Row>
    ...
    return (
        <React.Fragment>
            <div className="sticky-date">
                {emailListPosition === '0px' ?
                    <span className="email-group-header-date">{group.group} </span>
                    : null
                }
            </div>
            <ul className="no-padding mail-list-ul">
                {emailItem}
            </ul>
        </React.Fragment>
    )
